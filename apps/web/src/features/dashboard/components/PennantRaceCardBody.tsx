...
export type Heat = 'tight' | 'close' | 'comfortable';
...
export interface WildcardTeam extends RaceTeam {
  gamesBack: number;
  inWildcard: boolean;
}
...
export default function PennantRaceCardBody({
  loading,
  divisionRaces,
  wildcardRaces,
}: {
  ...
  return (
    <div className="mt-4 space-y-4">
      {divisionRaces.length > 0 ? <DivisionRaceStack divisionRaces={divisionRaces} /> : null}
      {wildcardRaces.length > 0 ? <WildcardRaceStack wildcardRaces={wildcardRaces} /> : null}
    </div>
  );
}

function DivisionRaceStack({ divisionRaces }: { divisionRaces: DivisionRace[] }) {
  return (
...
function WildcardRaceStack({ wildcardRaces }: { wildcardRaces: WildcardRace[] }) {
  return (
    <div className="space-y-2">
      <div className="font-data text-[11px] uppercase tracking-[0.16em] text-dynasty-muted">
        Wildcard Bubble
              ...
              >
                ...
                <span
                  className={`font-heading text-sm ${
                    team.inWildcard ? 'text-dynasty-textBright' : 'text-dynasty-text'
                  }`}
                >
                  {team.abbreviation}
                </span>
