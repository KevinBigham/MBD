...
import { Activity, AlertTriangle, ArrowRight, CheckCircle, Radio, TrendingDown, TrendingUp, X } from 'lucide-react';
...
interface PulseContentPanelProps {
  decisions: readonly DecisionSpotlightItem[];
  hasContent: boolean;
  leagueEvents: readonly LeagueEvent[];
  onAcknowledgeReport: () => void | Promise<void>;
  onDismissDecision: (decisionId: string) => void | Promise<void>;
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.keyInjuries.length > 0 && (
            <div className="space-y-1">
              ...
              {report.keyInjuries.map((injury, index) => (
                <div key={index} className="font-data text-xs text-dynasty-muted">{injury}</div>
              ))}
            </div>
          )}
          {report.keyReturns.length > 0 && (
            <div className="space-y-1">
              ...
              {report.keyReturns.map((returningPlayer, index) => (
                <div key={index} className="font-data text-xs text-dynasty-muted">{returningPlayer}</div>
              ))}
            </div>
          )}
        </div>
...
function LeagueEventsSection({ events }: { events: readonly LeagueEvent[] }) {
  if (events.length === 0) {
    return null;
  }
...
export function PulseContentPanel({
  decisions,
  hasContent,
  leagueEvents,
  onAcknowledgeReport,
    ...
    <div className="space-y-6">
      ...
      {decisions.length > 0 ? (
        <div className="space-y-3">
          <h2 className="font-heading text-sm text-dynasty-textBright">
            Decision Spotlights
            <span className="ml-2 font-data text-xs text-dynasty-muted">({decisions.length})</span>
          </h2>
          {decisions.map((decision) => (
            <DecisionCard
              key={decision.id}
              decision={decision}
              onDismiss={() => onDismissDecision(decision.id)}
            />
          ))}
        </div>
      ) : null}
