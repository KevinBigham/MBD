...
export default function PersonalityTab({
  ...
  return (
    <div className="space-y-6">
      {personalityProfile ? (
        <DensePanel title="Personality Profile" bodyClassName="space-y-4">
        ...
        <DensePanel title="Personality Profile" bodyClassName="p-8 text-center font-heading text-sm text-dynasty-muted">
          Personality telemetry is not available for this player right now.
        </DensePanel>
      )}

      {player.personalityTraits?.length ? (
        <DensePanel title="Traits">
