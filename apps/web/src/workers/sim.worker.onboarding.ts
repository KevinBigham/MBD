...
export function getDayOneSession(): DayOneSession {
  return buildDayOneSession();
}

export function advanceDayOneIntro(): WorkerMutationResult {
  ensureDayOneStep('owner_intro');
  const s = requireState();
  s.franchise.dayOne.currentStep = 'agm_select';
  s.franchise.dayOne.status = 'in_progress';
  return { success: true, flowStateChanged: true };
}

export function chooseDayOneAGM(agmId: AGMCandidateId): WorkerMutationResult {
  const s = requireState();
  ensureDayOneStep('agm_select');
  s.franchise.assistantGMId = agmId;
  s.franchise.dayOne.selectedAGMId = agmId;

  if (s.franchise.dayOne.experience === 'quick') {
    autoResolveQuickStart(agmId);
    return { success: true, flowStateChanged: true };
  }

  s.franchise.dayOne.currentStep = 'org_review';
  s.franchise.dayOne.status = 'in_progress';
  return { success: true, flowStateChanged: true };
}

export function advanceDayOneOrgReview(): WorkerMutationResult {
  ensureDayOneStep('org_review');
  const s = requireState();
  s.franchise.dayOne.currentStep = 'season_goal';
  return { success: true, flowStateChanged: true };
}

export function setDayOneSeasonGoal(seasonGoal: GMPhilosophy['seasonGoal']): WorkerMutationResult {
  ensureDayOneStep('season_goal');
  const s = requireState();
  s.franchise.dayOne.seasonGoal = seasonGoal;
  s.franchise.dayOne.currentStep = 'budget';
  syncFranchisePhilosophyFromDayOne();
  return { success: true, flowStateChanged: true };
}

export function setDayOneBudgetAllocation(budgetAllocation: DayOneBudgetAllocation): WorkerMutationResult {
  ensureDayOneStep('budget');
  const s = requireState();
  s.franchise.dayOne.budgetAllocation = budgetAllocation;
  s.franchise.dayOne.currentStep = 'opening_day_plan';
  syncFranchisePhilosophyFromDayOne();
  return { success: true, flowStateChanged: true };
}

export function setDayOneOpeningPlan(plan: DayOneOpeningPlan): WorkerMutationResult {
  ensureDayOneStep('opening_day_plan');
  const s = requireState();
  const defaults = buildOpeningDayPlan(s.players.filter((player) => player.teamId === s.userTeamId));
  s.franchise.dayOne.openingDayPlan = normalizeOpeningDayPlan(plan, defaults);
  s.franchise.dayOne.currentStep = 'development';
  return { success: true, flowStateChanged: true };
}

export function setDayOneDevelopmentPlan(plan: DayOneDevelopmentPlanInput): WorkerMutationResult {
  ensureDayOneStep('development');
  const s = requireState();
  s.franchise.dayOne.developmentStyle = plan.developmentStyle;
  s.franchise.dayOne.promotionStance = plan.promotionStance;
  const orgReview = buildDayOneOrgReview(s.players, s.userTeamId);
  const crisis = buildCurrentCrisis(orgReview);
  s.franchise.dayOne.crisisType = crisis?.type ?? null;
  s.franchise.dayOne.currentStep = 'crisis';
  syncFranchisePhilosophyFromDayOne();
  return { success: true, flowStateChanged: true };
}

export function resolveDayOneCrisis(responseId: string): WorkerMutationResult {
  ensureDayOneStep('crisis');
  const s = requireState();
  const orgReview = buildDayOneOrgReview(s.players, s.userTeamId);
  const crisis = buildCurrentCrisis(orgReview);
  const validResponse = crisis?.responseOptions.some((option) => option.id === responseId);
  if (!validResponse) {
    throw new Error(`Unknown Day One crisis response: ${responseId}`);
  }
  s.franchise.dayOne.crisisType = crisis?.type ?? s.franchise.dayOne.crisisType;
  s.franchise.dayOne.crisisResponseId = responseId;
  s.franchise.dayOne.currentStep = 'recap';
  return { success: true, flowStateChanged: true };
}

export function finishDayOne(): WorkerMutationResult {
  const s = requireState();
  if (s.franchise.dayOne.currentStep !== 'recap' && s.franchise.dayOne.currentStep !== 'complete') {
    throw new Error(`Cannot finish Day One from step ${s.franchise.dayOne.currentStep}.`);
  }

  s.franchise.dayOne.status = 'complete';
  s.franchise.dayOne.currentStep = 'complete';
  if (s.franchise.dayOne.experience === 'quick') {
    s.franchise.dayOne.quickStartRecapSeen = true;
  }
